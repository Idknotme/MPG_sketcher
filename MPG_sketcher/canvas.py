...
class sketch:
    ...
    def draw_fn(self, url_file, mode=1, co=(0, 0, 0), thickness=1, retain=False):
...
# Read the file content from the URL
        ...
        if data is None:
            ...
            data = open(f'{url_file}.txt', 'r')
            coord = self.get_coord(data)
            
        else:
            coord = self.get_coord_list(data)
